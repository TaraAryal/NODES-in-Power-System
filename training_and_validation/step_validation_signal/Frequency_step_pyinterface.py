"""
Filename:       Frequency_step_pyinterface.py
...
class Frequency_step:
    def __init__(self, filename: str = "Frequency_step_lib", compile: bool = False):
        """
        Class to simulate the Frequency_step system. The model input and output are shown below:
        ...
        if compile:
            compile_cmd = "gcc -fPIC -shared -DRT -O3 -lm " + '"' +\
                os.path.join(filepath, 'Frequency_step_interface.c') + '"' + ' "' +\
        ...
        self.Frequency_step_system = ctypes.cdll.LoadLibrary(
            os.path.join(filepath, filename))

        self.Frequency_step_system.initialize.restype = None
        self.Frequency_step_system.initialize.argtypes = [
        ...
        self.Frequency_step_system.one_step.restype = None
        self.Frequency_step_system.one_step.argtypes = [
            np.ctypeslib.ndpointer(dtype=np.float64),
            np.ctypeslib.ndpointer(dtype=np.float64),
            np.ctypeslib.ndpointer(dtype=np.float64),
    ...
    def reset(self, ):
        ...
        self.Frequency_step_system.initialize(
    ...
    def one_step(self, ):
        ...
        self.Frequency_step_system.one_step(
            self.inputpower1, self.deltaomega, self.deltaomegadot)
    ...
    def terminate(self):
        ...
        self.Frequency_step_system.terminate()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="Frequency_step_pyinterface",
        description="Python interface to Frequency_step.",
    )
    parser.add_argument(
    ...
    if args.stop_time is None:
        raise Exception(
            "Stop time not specified. Type `python Frequency_step_pyinterface.py --help` for more info!!!")
    ...
    m = Frequency_step("Frequency_step_lib")
