...
class Frequency_square:
    def __init__(self, filename: str = "Frequency_square_lib", compile: bool = False):
        """
        ...
        if compile:
            compile_cmd = "gcc -fPIC -shared -DRT -O3 -lm " + '"' +\
                os.path.join(filepath, 'Frequency_square_interface.c') + '"' + ' "' +\
        ...
        self.Frequency_square_system = ctypes.cdll.LoadLibrary(
            os.path.join(filepath, filename))

        self.Frequency_square_system.initialize.restype = None
        self.Frequency_square_system.initialize.argtypes = [
        ...
        self.Frequency_square_system.one_step.restype = None
        self.Frequency_square_system.one_step.argtypes = [
            np.ctypeslib.ndpointer(dtype=np.float64),
            np.ctypeslib.ndpointer(dtype=np.float64),
            np.ctypeslib.ndpointer(dtype=np.float64),
    ...
    def reset(self, ):
        ...
        self.Frequency_square_system.initialize(
    ...
    def one_step(self, ):
        ...
        self.Frequency_square_system.one_step(
            self.inputpower1, self.deltaomega, self.deltaomegadot)
    ...
    def terminate(self):
        ...
        self.Frequency_square_system.terminate()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="Frequency_square_pyinterface",
        description="Python interface to Frequency_square.",
    )
    parser.add_argument(
    ...
    if args.stop_time is None:
        raise Exception(
            "Stop time not specified. Type `python Frequency_square_pyinterface.py --help` for more info!!!")
    ...
    m = Frequency_square("Frequency_square_lib")
