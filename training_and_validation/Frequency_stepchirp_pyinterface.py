"""
Filename:       Frequency_stepchirp_pyinterface.py
...
class Frequency_stepchirp:
    def __init__(self, out_lib_name: str="Frequency_stepchirp_lib.so", compile: bool=True):
        """
        Class to simulate the Frequency_stepchirp system. The model input and output are shown below:
        ...
        if compile:
            compile_cmd = "gcc -fPIC -shared -O3 -lm " + '"' +\
                os.path.join(filepath, 'Frequency_stepchirp_interface.c') + '"' + ' "' +\
        ...
        self.Frequency_stepchirp_system = ctypes.cdll.LoadLibrary(os.path.join(filepath, out_lib_name))

        self.Frequency_stepchirp_system.initialize.restype = None
        self.Frequency_stepchirp_system.initialize.argtypes = [
        ...
        self.Frequency_stepchirp_system.one_step.restype = None
        self.Frequency_stepchirp_system.one_step.argtypes = [
            np.ctypeslib.ndpointer(dtype=np.float64),
            np.ctypeslib.ndpointer(dtype=np.float64),
            np.ctypeslib.ndpointer(dtype=np.float64),
    ...
    def reset(self, ):
        ...
        self.current_time = self.start_time
        self.next_time = self.current_time + self.step_time
        self.Frequency_stepchirp_system.initialize( self.inputpower1, self.deltaomega, self.deltaomegadot)
    ...
    def one_step(self, ):
        ...
        """
        self.Frequency_stepchirp_system.one_step( self.inputpower1, self.deltaomega, self.deltaomegadot)
...
#         prog="Frequency_stepchirp_pyinterface",
#         description="Python interface to Frequency_stepchirp.",
#     )
...
#         raise Exception("Stop time not specified. Type `python Frequency_stepchirp_pyinterface.py --help` for more info!!!")
...
#     m = Frequency_stepchirp("Frequency_stepchirp_lib.so", step_time=STEP_TIME, compile=True)
...
#     hf = h5py.File("Frequency_stepchirp_out.h5", "w")
